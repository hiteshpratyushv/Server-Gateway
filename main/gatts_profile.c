...
next: ;
       ...
       // xTaskCreate(&mqtt_task, "mqtt_task", 10240, buf, 5, NULL);
            ...
            if (adv_name != NULL) {
              ...
              /*
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ESP_LOGI(GATTS_TAG, "searched device %s\n", remote_device_name);
                    if (connect == false) {
                        connect = true;
                        ESP_LOGI(GATTS_TAG, "connect to the remote device.");
          ...
          //  printf("MMM: 111 ESP_GAP_BLE_SCAN_RESULT_EVT, evt : %d \n", scan_result->scan_rst.search_evt);
exit:       break;
